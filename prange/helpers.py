...
def systematic_prange_cost(alpha, n, k, w, eo, parameter):
    ...
    # Case 1: Infeasible configurations
    if (
        (n - k - alpha) < (w - eo)    # too few remaining coordinates
        or (w_r_perm > perm_r and w_r_perm >= 0)   # insufficient random-part columns to extract required errors
        or (w_l > perm_l)                  # insufficient identity-part columns to extract required errors
    ):
        cost = float("inf")
    ...
    # Case 2: Standard feasible cost computation
        ...
        cost = ( numerator - log2(comb(perm_l,w_l)) - log2(comb(perm_r, w_r_perm))
        )
